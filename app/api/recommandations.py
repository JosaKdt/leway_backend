...
GET  /api/recommandations/moi/top3 → top 3 + synthèse de la dernière recommandation
...
async def generer_recommandation(
    ...
    top3_valides   = veto["top5_valides"][:3]
    ...
    # IMPORTANT : on lit le mode LLM via pydantic_settings (settings.LLM_MODE),
    # PAS via os.getenv() directement, car pydantic_settings garantit le
    # chargement fiable du fichier .env à chaque démarrage de l'application.
    mode_llm = settings.LLM_MODE
    print(f"[LLM] Mode actif : {mode_llm}")
    rapport_llm: dict = {}
    ...
    recommandation = Recommandation(
        ...
        score_max=score_max,
        statut="generee",
        rapport_synthese=rapport_llm.get("rapport_synthese"),
    )
    ...
    for i, f in enumerate(top3_valides[:3], start=1):
        justif_obj = next(
            (x for x in rapport_llm.get("top3_justifiees", [])
             if x.get("nom") == f["nom"]),
            None,
        )
        session.add(ScoreCompatibilite(
            ...
            classement         = i,
            justification_ia   = justif_obj.get("justification") if justif_obj else None,
            points_forts       = justif_obj.get("points_forts") if justif_obj else None,
            points_attention   = justif_obj.get("points_attention") if justif_obj else None,
        ))
...
@router.get(
    "/moi/top3",
    summary="Top 3 filières + synthèse de ma dernière recommandation",
)
def get_mon_top3(
    session: Session = Depends(get_session),
    current_user=Depends(get_current_user),
    ...
    if not recommandation:
        raise HTTPException(status_code=404, detail="Aucune recommandation")

    profil = session.exec(
        select(ProfilPsychometrique)
        .where(ProfilPsychometrique.id_bachelier == id_bachelier)
    ).first()

    scores = session.exec(
        select(ScoreCompatibilite)
        .where(
            ScoreCompatibilite.id_recommandation == recommandation.id_recommandation,
            ScoreCompatibilite.classement != None,
        )
        .order_by(ScoreCompatibilite.classement)
    ).all()

    return {
        "id_recommandation": str(recommandation.id_recommandation),
        "date_generation":   recommandation.date_generation.isoformat() if recommandation.date_generation else None,
        "rapport_synthese":  recommandation.rapport_synthese,
        "scores_riasec": {
            "R": profil.score_r if profil else 0,
            "I": profil.score_i if profil else 0,
            "A": profil.score_a if profil else 0,
            "S": profil.score_s if profil else 0,
            "E": profil.score_e if profil else 0,
            "C": profil.score_c if profil else 0,
        },
        "top3": [
            {
                "id":               str(s.id_filiere),
                "classement":       s.classement,
                "weighted":         s.score_weighted,
                "riasec_match":     s.score_riasec_match or 0.0,
                "justification":    s.justification_ia or "",
                "points_forts":     s.points_forts or [],
                "points_attention": s.points_attention or [],
            }
            for s in scores
        ],
    }
